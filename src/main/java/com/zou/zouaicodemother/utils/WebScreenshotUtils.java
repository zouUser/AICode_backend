...
public class WebScreenshotUtils {
	...
	static {
		...
		System.setProperty("wdm.chromeDriverMirrorUrl", "https://npmmirror.com/mirrors/chromedriver/");
	}

	/**
	 * 获取 WebDriver 实例（延迟初始化，线程安全）
	 */
	...
	/**
	 * 关闭 WebDriver（由 Spring 容器管理时通过 @PreDestroy 调用）
	 */
	@PreDestroy
	public void destroy() {
		if (webDriver != null) {
	...
	/**
	 * 初始化 Chrome 浏览器驱动
	 * 按优先级查找 Chrome 二进制：系统属性 > 环境变量 > 常见路径 > Selenium 自动发现
	 */
	private static WebDriver initChromeDriver(int width, int height) {
		try {
			...
			if (chromeBinary != null) {
				...
				log.info("使用 Chrome 二进制路径: {}", chromeBinary);
			}
			// 无头模式
	...
	/**
	 * 查找 Chrome/Chromium 二进制文件路径
	 * 优先级：系统属性 chrome.binary > 环境变量 CHROME_BINARY > 常见安装路径
	 */
	private static String findChromeBinary() {
		// 1. 系统属性
		String prop = System.getProperty("chrome.binary");
		if (StrUtil.isNotBlank(prop) && new File(prop).exists()) {
			return prop;
		}
		// 2. 环境变量
		String env = System.getenv("CHROME_BINARY");
		if (StrUtil.isNotBlank(env) && new File(env).exists()) {
			return env;
		}
		// 3. 常见安装路径
		String[] candidatePaths = {
			"D:/java_environment/chrome/chrome-win64/chrome.exe",
